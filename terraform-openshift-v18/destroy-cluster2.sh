...
# tfvars file is REQUIRED (via argument or env variable)
if [[ -z "$TFVARS_FILE" ]]; then
    TFVARS_FILE="${TF_VAR_FILE:-}"
fi
if [[ -z "$TFVARS_FILE" ]]; then
    echo -e "${RED}Error: tfvars file is required${NC}"
    echo ""
    echo "Usage: $0 --var-file=<tfvars-file> [--dry-run] [--auto-approve]"
    echo "   or: $0 <tfvars-file> [--dry-run] [--auto-approve]"
    echo "   or: TF_VAR_FILE=<tfvars-file> $0 [--dry-run] [--auto-approve]"
    echo ""
    echo "Example:"
    echo "  $0 --var-file=env/my-cluster.tfvars --dry-run"
    exit 1
fi
